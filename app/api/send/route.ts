...
export async function POST(req: Request) {
  try {
    ...
    if (conversation.control_mode !== 'human') return NextResponse.json({ error: 'Take the conversation before sending a human reply' }, { status: 409 });
    ...
    const { error: insertError } = await db.from('messages').insert({ conversation_id: conversation.id, role: 'human', content: text.trim() });
    if (insertError) return NextResponse.json({ error: 'Message was sent but history save failed' }, { status: 500 });
